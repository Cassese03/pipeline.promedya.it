    ...
    public function statistiche(Request $request, $data = 0)
    {
        ...
        if (session()->has('utente')) {
            ...
            if ($data == 0) {
                ...
                $statistiche_corrente = DB::select('SELECT f.Vinta, SUM(Val) AS Val FROM (SELECT IF(Vinta = 2,\'Vinta\',IF(Vinta = 1,\'Persa\',\'In corso\')) AS Vinta,CAST(SUM(Val_Ven_AC) as Decimal(20,2)) as Val
                                                      FROM   pipeline
