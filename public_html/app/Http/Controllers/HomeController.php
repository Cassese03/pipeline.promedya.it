    ...
    function statistiche(Request $request, $data = 0)
    {
        ...
        if (session()->has('utente')) {
            ...
            if ($data == 0) {
                ...
                if ($semestre == 2)
                    $statistiche_incentivi = DB::SELECT('(SELECT Coalesce(SUM(Vendita_Budget),0) as valore,\'Vendite_OLD\' as type FROM pipeline where Vinta = 2 and Data_Probabile_Chiusura <= CONCAT(YEAR(CURDATE()),\'1231\') and Data_Probabile_Chiusura >= CONCAT(YEAR(CURDATE()),\'0701\') and Prodotto not in (SELECT Descrizione from prodotto where sottogruppo = \'SERVIZI\' or gruppo = \'ARCA SVILUPPO\') and Tipo_Cliente = \'OLD\') UNION ALL (SELECT Coalesce(SUM(Vendita_Budget),0) as valore,\'Vendite_NEW\' as type FROM pipeline where Vinta = 2 and Data_Probabile_Chiusura <= CONCAT(YEAR(CURDATE()),\'1231\') and Data_Probabile_Chiusura >= CONCAT(YEAR(CURDATE()),\'0701\') and Prodotto not in (SELECT Descrizione from prodotto where sottogruppo = \'SERVIZI\' or gruppo = \'ARCA SVILUPPO\') and Tipo_Cliente = \'LEAD\') UNION ALL (SELECT Coalesce(SUM(Vendita_Budget),0) as valore,\'SERVIZI\' as type FROM pipeline where Vinta = 2 and Data_Probabile_Chiusura <= CONCAT(YEAR(CURDATE()),\'1231\') and Data_Probabile_Chiusura >= CONCAT(YEAR(CURDATE()),\'0701\') and Prodotto in (SELECT Descrizione from prodotto where sottogruppo = \'SERVIZI\' or gruppo = \'ARCA SVILUPPO\')) ORDER BY Type desc');
                else
                    $statistiche_incentivi = DB::SELECT('(SELECT Coalesce(SUM(Vendita_Budget),0) as valore,\'Vendite_OLD\' as type FROM pipeline where Vinta = 2 and Data_Probabile_Chiusura <= CONCAT(YEAR(CURDATE()),\'0630\') and Data_Probabile_Chiusura >= CONCAT(YEAR(CURDATE()),\'0101\') and Prodotto not in (SELECT Descrizione from prodotto where sottogruppo = \'SERVIZI\' or gruppo = \'ARCA SVILUPPO\') and Tipo_Cliente = \'OLD\') UNION ALL (SELECT Coalesce(SUM(Vendita_Budget),0) as valore,\'Vendite_NEW\' as type FROM pipeline where Vinta = 2 and Data_Probabile_Chiusura <= CONCAT(YEAR(CURDATE()),\'0630\') and Data_Probabile_Chiusura >= CONCAT(YEAR(CURDATE()),\'0101\') and Prodotto not in (SELECT Descrizione from prodotto where sottogruppo = \'SERVIZI\' or gruppo = \'ARCA SVILUPPO\') and Tipo_Cliente = \'LEAD\') UNION ALL (SELECT Coalesce(SUM(Vendita_Budget),0) as valore,\'SERVIZI\' as type FROM pipeline where Vinta = 2 and Data_Probabile_Chiusura <= CONCAT(YEAR(CURDATE()),\'0630\') and Data_Probabile_Chiusura >= CONCAT(YEAR(CURDATE()),\'0101\') and Prodotto in (SELECT Descrizione from prodotto where sottogruppo = \'SERVIZI\' or gruppo = \'ARCA SVILUPPO\')) ORDER BY Type desc');
                ...
                $statistiche_corrente_sottogruppo_annuale =
                    DB::select('SELECT CAST(SUM(Val_Ven_AC) as Decimal(20,2)) as Val,(SELECT sottogruppo from prodotto where descrizione = pipeline.Prodotto) as gruppo,
                                                      ...
                                                      ORDER  BY CAST(SUM(Val_Ven_AC) as Decimal(20,2)) desc ');
                /*ORDER  BY CAST(SUM(Val_Ven_AC) as Decimal(20,2)) desc');*/
                $statistiche_sales_vinte_zona = DB::SELECT('SELECT * from (select o.Gruppo as Sales,CAST(SUM(Vendita_Budget) as Decimal(20,2)) as Val,(SELECT SUM(Vendita_Budget) from pipeline where Vinta = 2 and Data_Probabile_Chiusura >= CONCAT(YEAR(CURDATE()),\'-01-01\') and Data_Probabile_Chiusura <= CONCAT(YEAR(CURDATE()),\'-12-31\')) as Percentuale from pipeline left join  operatori o on o.username = pipeline.Sales where Vinta = 2 and Data_Probabile_Chiusura >= CONCAT(YEAR(CURDATE()),\'-01-01\') and Data_Probabile_Chiusura <= CONCAT(YEAR(CURDATE()),\'-12-31\') group by o.Gruppo) f order by f.Val DESC');
                $statistiche_sales_vinte = DB::TABLE('pipeline')->select(DB::raw('Sales,CAST(SUM(Vendita_Budget) as Decimal(20,2)) as Val'))->where('Vinta', '=', '2')->where('Data_Probabile_Chiusura', '>=', date('Y', strtotime('now')) . '-01-01')->where('Data_Probabile_Chiusura', '<=', date('Y', strtotime('now')) . '-12-31')->groupBy('Sales')->orderByDesc('Val')->get();
                ...
                $statistiche_corrente_sales = DB::select('SELECT CAST(SUM(Val_Ven_AC) as Decimal(20,2)) as Val,Sales
                                                      ...
                                                      ORDER  BY CAST(SUM(Val_Ven_AC) as Decimal(20,2)) desc');
                $statistiche_corrente_prodotto_annuale = DB::select('SELECT CAST(SUM(Val_Ven_AC) as Decimal(20,2)) as Val,(SELECT gruppo from prodotto where descrizione = pipeline.Prodotto) as gruppo,
                                                      (SELECT SUM(Val_Ven_AC) from pipeline WHERE  ((Vinta = 2) and DATE_FORMAT(Data_Probabile_Chiusura,\'%Y\') = DATE_FORMAT(\'' . $data . '\',\'%Y\'))) as Percentuale
                                                      FROM   pipeline
                ...
                $statistiche_sales_vinte = DB::TABLE('pipeline')->select(DB::raw('Sales,CAST(SUM(Vendita_Budget) as Decimal(20,2)) as Val'))->where('Vinta', '=', '2')->where('Data_Probabile_Chiusura', '>=', date('Y', strtotime($data)) . '-01-01')->where('Data_Probabile_Chiusura', '<=', date('Y', strtotime($data)) . '-12-31')->groupBy('Sales')->orderByDesc('Val')->get();
                $statistiche_sales_vinte_zona = DB::SELECT('SELECT * from (select o.Gruppo as Sales,CAST(SUM(Vendita_Budget) as Decimal(20,2)) as Val,(SELECT SUM(Vendita_Budget) from pipeline where Vinta = 2 and Data_Probabile_Chiusura >= CONCAT(YEAR(\'' . $data . '\'),\'-01-01\')  and Data_Probabile_Chiusura <= CONCAT(YEAR(CURDATE()),\'-12-31\')) as Percentuale from pipeline left join  operatori o on o.username = pipeline.Sales where Vinta = 2 and Data_Probabile_Chiusura >= CONCAT(YEAR(\'' . $data . '\'),\'-01-01\') and Data_Probabile_Chiusura <= CONCAT(YEAR(\'' . $data . '\'),\'-12-31\') group by o.Gruppo) f order by f.Val DESC');
