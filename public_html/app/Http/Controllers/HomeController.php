    ...
    function statistiche(Request $request, $data = 0)
    {
        ...
        if (session()->has('utente')) {
            ...
            if ($data == 0) {
                ...
                $incentivi = DB::select('SELECT * FROM incentivi where anno = YEAR(CURDATE()) and semestre = ' . $semestre);
                ...
                $incentivi = DB::select('SELECT * FROM incentivi where anno = YEAR(\'' . $data . '\') and semestre = ' . $semestre);
