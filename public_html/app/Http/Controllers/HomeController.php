    ...
    public function pipeline(Request $request)
    {
        ...
        if (isset($dati['aggiungi'])) {
            ...
            if ($dati['Vinta'] == 0 || $dati['Vinta'] == 1) {
                ...
                $mail->IsHTML(true);
                $mail->Subject = 'PROMEDYA SRL - SALES FORCE';
        ...
        if (session()->has('utente')) {
            ...
            if (isset($dati['filtra'])) {
                ...
                return View::make('rows', compact('utente', 'segnalato', 'motivazione', 'prodotto', 'dipendenti', 'rows', 'operatori', 'column', 'clienti', 'gruppo'));
            }
            ...
            $gruppo = DB::select('SELECT p.gruppo ,GROUP_CONCAT(p.descrizione)  as prodotti FROM prodotto p GROUP BY p.gruppo ');
            $clienti = DB::select('select Ragione_Sociale from pipeline group by Ragione_Sociale order by Ragione_Sociale ASC');
            return View::make('rows', compact('utente', 'rows', 'motivazione', 'prodotto', 'dipendenti', 'operatori', 'segnalato', 'column', 'clienti', 'gruppo'));
        } else {
